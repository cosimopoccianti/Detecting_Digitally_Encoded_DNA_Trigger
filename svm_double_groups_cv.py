...
def main(fragment_len, retention_pos, encryption_key, dataset_type, algo, full, clean, trojan, k_list, metrics_columns):
    ...
    metrics_folder = f"/home/cosimo/Desktop/PhD/Cyberbiosecurity/DNA_attacks/metrics/{algo}/Balanced/{dataset_type}/GroupKFold/fragment_len_{fragment_len}/"
    os.makedirs(metrics_folder, exist_ok=True)

    metrics_csv = os.path.join(metrics_folder, 
                            f"metrics_len_{fragment_len}_pos_{retention_pos}_key_{encryption_key}_double_cv(5,4)_dataset_len_{len(full)}_{dataset_type}_GKFold.csv")
